...
/**
 * 실무 사용 사례
 * 전화번호 입력 받을 떄 - 없어야 하면 걸러내야하기 때문에 '010-1234-5678'이라는 string 값(number라면 010이 불가능합니다)에 .indexOf('-')를 확인하여 -1이 return되면 다음 단계로, 3번지가 출력된다면 '-'을 제거하는 메서드로 넘길 수 있습니다.
 */
...
// python에서는 마이너스인덱스가 있기는 한데, JS에서는 없습니다. 하지만 silice()의 argument로 마이너스값을 넣어줄 수는 있습니다.
console.log(strFruits.slice(-12)); // 결과값 : Banana, Kiwi
/*
시작점이 -12번지고, 그 다음이 -11, -10, ... -1(즉 끝)까지 추출하는 메서드였습니다.
근데 -1 하니까 i가 짤리는데 이거 좀 나중에 처리하겠습니다.
*/
...
/**
 * 얘는 7번지부터 6개의 문자를 뽑아내겠다는 겁니다. B부터 시작해서 anana하면 전체 6개의 문자
 */
