...
08_array_destructuring

# JavaScript 주요 Web APIs
## LocalStorate / SessionStorage

1. LocalStorage
  - 저장된 데이터를 삭제하기 전까지 영구히 보전됨. 저장되는 모든 데이터는 JS 객체처럼 key - value pair로 이루어져있습니다. 단, 저장되는 모든 데이터 값은 string으로만 저장가능합니다. 그래서 이 저장소에 number, object, boolean, array 등과 같은 string이 아닌 데이터를 저장할 때는 JSON.stringify를 통해 문자열로 변환하여 저장해야 합니다. 그리고 localStorage의 데이터를 JS로 읽고 싶다면 JSON.parse()를 사용하여 객체로 변환해줘야 연산이 가능합니다.
...
- 이상의 코드의 주의 사항으로는 localStorage를 참조해야 하기 떄문에 html 파일도 요구됩니다. live server에서 확인할 것.
...
- todolist.html을 기준으로 보냈기때문에 00.css와 js가 기존 버전으로 되어있어서 htmlaks BEM 버전으로 바꾼다고 하더라도 실제로는 css가 적용되지 않습니다.

- 특정 방법을 사용했을 떄 여러분이 작성한거 다 날리고 덮어쓸 수도 있겠지만 남겨놓고 CSS 버전만 다양하게 갈아끼우고 싶다면 남겨두는 방법도 필요합니다.
